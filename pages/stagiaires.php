...
<?php
	...
	if($idf==0){
		$resultat = $con->query("SELECT S.ID,NOM,PRENOM,PHOTO,NOM_FILIERE,DATE_DEBUT,DATE_FIN FROM STAGIAIRE S,FILIERE F
								WHERE S.ID_FILIERE=F.ID AND (NOM like '%$mc%' OR PRENOM like '%$mc%')
								ORDER BY S.ID LIMIT $size OFFSET $offset");

		$resultat2 = $con->query("select count(*) as nbrSTAGAIRE from STAGIAIRE 
								where NOM like '%$mc%' OR PRENOM like '%$mc%'");
	}
	else{
		$resultat = $con->query("SELECT S.ID,NOM,PRENOM,PHOTO,NOM_FILIERE,DATE_DEBUT,DATE_FIN FROM STAGIAIRE S,FILIERE F
								WHERE S.ID_FILIERE=F.ID	AND (NOM like '%$mc%' OR PRENOM like '%$mc%')
								And ID_FILIERE=$idf	ORDER BY S.ID LIMIT $size OFFSET $offset");

		$resultat2 = $con->query("select count(*) as nbrSTAGAIRE from STAGIAIRE 
								where (NOM like '%$mc%' OR PRENOM like '%$mc%')	And ID_FILIERE=$idf");
	}
				...
				<div class="panel panel-success espace60">
					...
					<div class="panel-body">
						...
						<div class="form-group">						
								<select name="ID_FILIERE" id="ID_FILIERE" class="form-control" onChange="this.form.submit();">
