<?php
	...
	if (empty($validationErrors)) {
		...
		if (findUserByLogin($login) == 0 && findUserByEmail($email) == 0) {
			$stmt = $con->prepare('INSERT INTO UTILISATEUR(login, pwd,role, email,etat) VALUES (:pLogin,:pPwd,:pRole, :pEmail,:pEtat)');						
			$stmt->execute(array( 'pLogin' => $login, 'pPwd' => $pwd1,'pRole'=>'VISITEUR','pEmail'=> $email,'pEtat'	=>0));
