...
<?php
	...
	if($niveau=="all"){
		$resultat1 = $con->query("SELECT * FROM FILIERE WHERE  NOM_FILIERE like '%$mc%' 
								ORDER BY ID LIMIT $size OFFSET $offset");

		$resultat2 = $con->query("select count(*) as nbrFiliere from FILIERE 
								where NOM_FILIERE like '%$mc%'");
	}else{
		$resultat1 = $con->query("SELECT * FROM FILIERE WHERE  NOM_FILIERE like '%$mc%'AND NIVEAU='$niveau'
								ORDER BY ID	LIMIT $size	OFFSET $offset");

		$resultat2 = $con->query("select count(*) as nbrFiliere from FILIERE 
								where NOM_FILIERE like '%$mc%' AND NIVEAU='$niveau'");
	}
	$nbr=$resultat2->fetch();
	$nbrF=$nbr['nbrFiliere'];
	$reste=$nbrF % $size;
